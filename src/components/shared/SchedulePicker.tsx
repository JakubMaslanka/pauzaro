...
export function SchedulePicker({
	...
	const isDuplicateSlot = useCallback(
		(startTime: string, endTime: string, excludeIndex: number) => {
			return times.some(
				(slot, i) =>
					i !== excludeIndex &&
	...
	const addTimeSlot = useCallback(() => {
		const newSlot = { start_time: "09:00", end_time: "09:15" };
		if (isDuplicateSlot(newSlot.start_time, newSlot.end_time, -1)) {
			// Find a non-duplicate time
			const hours = [10, 11, 14, 15, 16, 12, 13, 17];
			for (const h of hours) {
				const start = `${String(h).padStart(2, "0")}:00`;
				const end = `${String(h).padStart(2, "0")}:15`;
				if (!isDuplicateSlot(start, end, -1)) {
					onTimesChange([...times, { start_time: start, end_time: end }]);
					return;
				}
			}
		}
	...
	return (
		<Stack gap="md">
			...
			<div>
				...
				<Stack gap="xs">
					{times.map((slot, idx) => {
						const duplicate = isDuplicateSlot(
							slot.start_time,
							slot.end_time,
							idx,
						);
						return (
							<Group key={slotKeys[idx]} gap="xs" align="center">
								...
								<input
									...
									value={slot.end_time}
									onChange={(e) =>
										updateTimeSlot(idx, "end_time", e.target.value)
									}
									style={{
										padding: "6px 10px",
										border: duplicate
