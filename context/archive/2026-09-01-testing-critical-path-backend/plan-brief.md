...
Write Rust tests for the two highest-risk backend scenarios: streak calculation edge cases and snooze 3x auto-fail enforcement. These are the densest untested business logic paths — test-plan Phase 1 (T-01) covers risks #2, #3 at the cheapest layer.

## Starting Point

`calculate_streak()` has 8 unit tests covering core paths. Snooze logic (`snooze_habit()`, `mark_done()`) and `PendingTriggerRepository` have zero tests. Backend uses `chrono::Local` everywhere — local time is the project convention (design decision 2026-09-02). Integration test infrastructure exists (`db_integration.rs` with `setup_db()` pattern).

## Desired End State

12 streak unit tests (4 new edge cases), 8 overlay integration tests in a new file covering snooze auto-fail flow and mark_done paths. All tests use `YYYY-MM-DD` date strings matching the local-time convention. `cargo test` passes green.
...
- `YYYY-MM-DD` local-time date strings as test inputs
...
- Test-plan risks #2, #3 have explicit test coverage matching the Risk Response Guidance table
